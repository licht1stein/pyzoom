...
class MeetingsComponent:
    ...
    def create_meeting(self, topic: str, *, start_time: str, duration_min: int, timezone: str = None,
                       type_: int = 2, password: str = None,
                       settings: schemas.ZoomMeetingSettings = None) -> schemas.ZoomMeeting:
        ...
        body = {
            ...
            'settings': settings.dict() if settings else schemas.ZoomMeetingSettings.default_settings().dict()
        }
    ...
    def list_meeting_registrants(self, meeting_id: int) -> schemas.MeetingRegistrantsList:
        endpoint = f'/meetings/{meeting_id}/registrants'
        return schemas.MeetingRegistrantsList(**self._client.get_all_pages(endpoint))

    def update_meeting_registrant_status(self, meeting_id: int, *, action: Literal['approve', 'cancel', 'deny'],
    ...
    def add_meeting_registrant(self, meeting_id: int, *, first_name: str, last_name: str, email: str,
                               **kwargs) -> schemas.RegistrantConfirmation:
        endpoint = f'/meetings/{meeting_id}/registrants'
        registrant = schemas.MeetingRegistrant(first_name=first_name, last_name=last_name, email=email, **kwargs)
        return schemas.RegistrantConfirmation(**self._client.post(endpoint, body=registrant.dict()).json())

    def add_and_confirm_registrant(self, meeting_id: int, *, first_name: str, last_name: str, email: str,
                                   **kwargs) -> schemas.RegistrantConfirmation:
        confirmation = self.add_meeting_registrant(meeting_id, first_name=first_name, last_name=last_name, email=email,
                                                   **kwargs)
        self.update_meeting_registrant_status(meeting_id, action='approve',
                                              registrants=[schemas.MeetingRegistrantShort(id=confirmation.registrant_id,
                                                                                          email=email)])
        return confirmation

    def cancel_registration(self, meeting_id: int, *, registrant_id: str, email: str):
        self.update_meeting_registrant_status(meeting_id, action='cancel',
                                              registrants=[schemas.MeetingRegistrantShort(id=registrant_id,
                                                                                          email=email)])
    ...
    def past_meeting_participants(self, meeting_id: int) -> schemas.MeetingParticipantList:
...
class ZoomClient:
    ...
    def __attrs_post_init__(self):
        self.raw: APIClientBase = APIClientBase(api_key=self.api_key, api_secret=self.api_secret)
